...
const TailorDashboard = () => {
  ...
  const { data: orders, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('http://alnubrasstudio.ddns.net:8888/tailor/orders', {
        credentials: 'include'
      })
  ...
  const { data: orderDetails, isLoading: isOrderDetailsLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`http://alnubrasstudio.ddns.net:8888/tailor/order/43543543`, {
        credentials: 'include'
      })
      return await response.json()
    }
