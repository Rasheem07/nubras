...
export default function UsersPage() {
  ...
  const { data: users, isLoading } = useQuery({
    queryKey: ['users'],
    queryFn: async () => {
      const response = await fetch("https://api.alnubrasstudio.com/admin/users", { credentials: 'include' });
  ...
  const { mutate: revokeUser, isPending } = useMutation({
    mutationFn: async (id: string) => {
      const response = await fetch(`https://api.alnubrasstudio.com/admin/revoke/${id}`, { credentials: 'include', method: 'POST' });
  ...
  const { mutate: reactivateUser, isPending: isActivating } = useMutation({
    mutationFn: async (id: string) => {
      const response = await fetch(`https://api.alnubrasstudio.com/admin/reactivate/${id}`, { credentials: 'include', method: 'POST' });
      const json = await response.json()
