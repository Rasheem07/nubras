...
export default function SupplierPage() {
    ...
    const { data: suppliers, isLoading } = useQuery({
        queryKey: ['suppliers'],
        queryFn: async () => {
            const response = await fetch('http://alnubras.dyndns.org:3000/inventory/suppliers', {
                credentials: 'include'
            })
    ...
    const createSupplier = async (data: any) => {
        const response = await fetch("http://alnubras.dyndns.org:3000/inventory/suppliers/add", {
            method: "POST",
            headers: {
