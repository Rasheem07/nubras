...
export default function ProductsPage() {
    ...
    const getSections = async () => {
        const response = await fetch('http://localhost:3000/service/sections', { headers: { "Content-Type": "application/json" }, credentials: 'include' });
        const data = await response.json();
        return Array.isArray(data) ? data : [];
    }

    const createSection = async (data: { name: string }) => {
        const response = await fetch('http://localhost:3000/service/add-section', {
            method: 'POST',
