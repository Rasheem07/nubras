...
export default function ProductsPage() {
    ...
    const getSections = async () => {
        const response = await fetch('https://api.alnubrasstudio.com/service/sections', { headers: { "Content-Type": "application/json" }, credentials: 'include' });
        const data = await response.json();
        return Array.isArray(data) ? data : [];
    }

    const createSection = async (data: { name: string }) => {
        const response = await fetch('https://api.alnubrasstudio.com/service/add-section', {
            method: 'POST',
