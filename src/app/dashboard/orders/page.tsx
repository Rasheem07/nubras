...
import { Loader2, Package, Truck, CheckCircle2, ClipboardCheck, PackageCheck, XCircle, Trash, ArrowLeft, Box, TruckIcon, Scissors, ShoppingCart, Calendar, TimerReset, IdCard, ChartArea, Badge, ShoppingBag, User, AlertOctagon } from 'lucide-react';
...
const fetchOrders = async (): Promise<Order[]> => {
  const response = await fetch('http://alnubras.dyndns.org:3000/orders', {
    credentials: 'include'
  });
  if (!response.ok) {
...
export default function OrdersPage() {
  ...
  const handleCancelOrder = async () => {
    ...
    const response = await fetch(`http://alnubras.dyndns.org:3000/orders/cancel/${selectedOrder.InvoiceId}`, {
      method: 'DELETE',
      credentials: 'include'
    });
    if (response.ok) {
  ...
  const handleUpdateOrderStatus = async () => {
    ...
    const response = await fetch(`http://alnubras.dyndns.org:3000/orders/update/${selectedOrder?.InvoiceId}`, {
      method: 'PUT',
      credentials: 'include',
      headers: {
  ...
  const handleDeleteOrder = async () => {
    const response = await fetch(`http://alnubras.dyndns.org:3000/orders/delete/${selectedOrder?.InvoiceId}`, {
      method: 'DELETE',
      credentials: 'include'
    });
    if (response.ok) {
  ...
  async function downloadInvoicePdf(invoiceId: number) {
    try {
      const response = await fetch(`http://alnubras.dyndns.org:3000/invoices/pdf/${invoiceId}`);
