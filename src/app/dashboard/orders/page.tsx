...
const fetchOrders = async (): Promise<Order[]> => {
  const response = await fetch('https://api.alnubrasstudio.com/orders', {
    credentials: 'include'
  });
  if (!response.ok) {
...
export default function OrdersPage() {
  ...
  const handleCancelOrder = async () => {
    ...
    const response = await fetch(`https://api.alnubrasstudio.com/orders/cancel/${selectedOrder.InvoiceId}`, {
      method: 'DELETE',
      credentials: 'include'
    });
    if (response.ok) {
  ...
  const handleUpdateOrderStatus = async () => {
    ...
    const response = await fetch(`https://api.alnubrasstudio.com/orders/update/${selectedOrder?.InvoiceId}`, {
      method: 'PUT',
      credentials: 'include',
      headers: {
  ...
  const handleDeleteOrder = async () => {
    const response = await fetch(`https://api.alnubrasstudio.com/orders/delete/${selectedOrder?.InvoiceId}`, {
      method: 'DELETE',
      credentials: 'include'
    });
    if (response.ok) {
  ...
  async function downloadInvoicePdf(invoiceId: number) {
    try {
      const response = await fetch(`https://api.alnubrasstudio.com/invoices/pdf/${invoiceId}`);
